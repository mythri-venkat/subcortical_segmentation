...
class CropToFixed:
    def __init__(self, random_state, size=(64,64,64), centered=False, **kwargs):
        self.random_state = random_state
    ...
    def __call__(self, m):
        ...
        if not self.centered:
            ...
            z_range, z_pad = _rand_range_and_pad(self.crop_z, z)

            y_start = self.random_state.randint(y_range)
            x_start = self.random_state.randint(x_range)
            z_start = self.random_state.randint(z_range)
...
class Transformer:
    ...
    def _transformer_class(class_name):
        m = importlib.import_module('unetseg3d.augment.transforms')
